...
// ---- Stat cards ----
const STATS = [
  { label: 'Open Cases', value: '1', accent: '#7C3AED', icon: '📁' },
  { label: 'Active Campaigns', value: '1', accent: '#2563EB', icon: '🎯' },
  { label: 'Compliance Score', value: '94%', accent: '#22C55E', icon: '✅' },
  { label: 'AI Quality', value: '87%', accent: '#14B8A6', icon: '🤖' },
]

// ---- Risk by site ----
const RISK_BY_SITE: { name: string; level: string; pct: number; color: string }[] =
  [
...
const FEEDBACK: {
  ...
  { label: 'correct_override', display: 'Correct override', pct: 77, color: '#22C55E' },
...
export default function ExecutiveReporting() {
  ...
  async function generateBrief() {
    ...
    try {
      ...
      setBrief(
        '⚠️ Unable to reach the executive briefing agent. Verify ANTHROPIC_API_KEY is configured in .env.local.'
      )
    } finally {
      setGenerating(false)
    }
    ...
    <div className="space-y-6 px-8 py-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {STATS.map((s) => (
          <div
            key={s.label}
            className="rounded-xl bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.10),0_1px_2px_rgba(0,0,0,0.06)]"
          >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.10),0_1px_2px_rgba(0,0,0,0.06)]">
          <h3 className="mb-4 text-sm font-bold text-[#111827]">Risk by Site</h3>
            ...
            <ul className="flex-1 space-y-2">
              {FEEDBACK.map((f) => (
                ...
                >
                  ...
                  <span className="font-semibold text-[#111827]">{f.pct}%</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="rounded-xl border border-[#FDE68A] bg-[#FFFBEB] p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-xs leading-relaxed text-[#92400E]/90">
              {campaign.hypothesis}
            </p>
            <div className="mt-3 flex flex-wrap gap-4 text-xs font-semibold text-[#B45309]">
      ...
      <div className="rounded-xl bg-white shadow-[0_1px_3px_rgba(0,0,0,0.10),0_1px_2px_rgba(0,0,0,0.06)]">
        ...
        <div ref={briefRef} className="max-h-80 overflow-y-auto p-5">
          {brief ? (
            <div className="space-y-1.5 text-sm leading-relaxed text-[#374151]">
              {brief.split('\n').map((line, i) => {
                if (line.startsWith('## ')) {
                  return (
                    <h4
                      key={i}
                      className="pt-2 text-xs font-bold uppercase tracking-wide text-[#7C3AED]"
                    >
                      {line.replace('## ', '')}
                    </h4>
                  )
