...
export function FloorPlanLeaflet({
  ...
  return (
    <div className="relative flex h-full min-h-[460px] flex-col overflow-hidden rounded-xl bg-[#0B0E14]">
      ...
      <div className="flex min-h-12 flex-wrap items-center gap-2 border-b border-[#273142] bg-[#111827] px-3 py-2 sm:gap-3 sm:px-4">
        <div className="min-w-[180px] flex-1">
          <p className="text-sm font-bold text-white">{floorPlan.label}</p>
        </div>
        <p className="order-3 w-full text-xs font-semibold text-[#94A3B8] sm:order-none sm:w-auto md:block">
          Pan and zoom · click cameras for live view
        </p>
        <div className="shrink-0 rounded-lg border border-[#334155] bg-[#0F172A] px-3 py-2 text-xs font-bold text-[#E5E7EB]">
          {visibleDevices.length} devices · {visibleIncidents.length} incidents
        </div>
      </div>

      <div className="floor-map min-h-0 flex-1">
        <MapContainer
          key={floorPlan.id}
          crs={L.CRS.Simple}
          bounds={bounds}
